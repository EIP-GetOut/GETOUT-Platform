...
describe('Books Routes', () => {
  ...
  it('should respond with 200 OK for GET /generate-books?intitle=fleurs', async () => {
    await request(app).get('/generate-books?intitle=fleurs').set('Cookie', cookie).then((response) => {
  ...
  it('should respond with 500 Bad Request', async () => {
    await request(app).get('/generate-books').set('Cookie', cookie).then((response) => {
