...
import { getRecommandationsFromHistory, generateRecommendationsFromScratch } from '@models/account/recommendMovies'
...
router.get('/account/:accountId/recommend-movies', logApiRequest, (req: Request, res: Response) => {
  ...
  const lastMovieRecommandation = req.session.account.lastMovieRecommandation

  if (lastMovieRecommandation == null) {
    logger.info('Last movie recommendation does not exist, generating new recommendations from scratch.')
  ...
  if (Date.now() - new Date(lastMovieRecommandation).getTime() >= interval) {
    getNextRecommendations(accountId).then(async (nextRecommendations): Promise<void> => {
      if (nextRecommendations !== null) {
        await setCurrentRecommendations(accountId, nextRecommendations).then(async () => {
          return await addRecommendedMoviesToHistory(nextRecommendations, accountId)
        }).then(async () => {
          await modifyAccount(accountId, { lastMovieRecommandation: new Date() })
        }).then(async () => {
    ...
    logger.debug(`Missing ${(interval - (Date.now() - new Date(lastMovieRecommandation).getTime())) / 1000} seconds to generate new recommendations.`)
    ...
    getCurrentRecommendations(accountId).then(async (cachedRecommendations): Promise<void> => {
      if (cachedRecommendations !== null && Date.now() - new Date(lastMovieRecommandation).getTime() < interval) {
